...
CREATE TABLE `list_product_types` (
  ...
  PRIMARY KEY (`uuid`,`product_type`),
  KEY `product_type_interface_id_creation` (`interface_id_creation`),
  KEY `product_type_interface_id_update` (`interface_id_update`),
  CONSTRAINT `product_type_interface_id_creation` FOREIGN KEY (`interface_id_creation`) REFERENCES `db_interfaces` (`uuid`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `product_type_interface_id_update` FOREIGN KEY (`interface_id_update`) REFERENCES `db_interfaces` (`uuid`) ON DELETE CASCADE ON UPDATE CASCADE
...
BEGIN
  ...
  VALUES('INSERT', NOW(), NEW.`uuid`, NEW.`interface_id_creation`, NEW.`interface_id_update`, NEW.`is_obsolete`, NEW.`order`, NEW.`product_type`, NEW.`product_type_description`)
  ;
END
$$
...
BEGIN
  INSERT INTO `logs_list_product_types` (
    ...
    VALUES
    ('UPDATE-OLD_VALUES', NOW(), OLD.`uuid`, OLD.`interface_id_creation`, OLD.`interface_id_update`, OLD.`is_obsolete`, OLD.`order`, OLD.`product_type`, OLD.`product_type_description`),
    ('UPDATE-NEW_VALUES', NOW(), NEW.`uuid`, NEW.`interface_id_creation`, NEW.`interface_id_update`, NEW.`is_obsolete`, NEW.`order`, NEW.`product_type`, NEW.`product_type_description`)
  ;
END
$$
...
BEGIN
  INSERT INTO `logs_list_product_types` (
    ...
    VALUES
    ('DELETE', NOW(), OLD.`uuid`, OLD.`interface_id_creation`, OLD.`interface_id_update`, OLD.`is_obsolete`, OLD.`order`, OLD.`product_type`, OLD.`product_type_description`)
  ;
END
$$
