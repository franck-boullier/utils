...
#
# - Create a table `list_merch_t_name_family_statuses` to list the possible statutes for a record.
# - Create a trigger `uuid_list_merch_t_name_family_statuses` to automatically generate the UUID for a new record.
# - Create a table `logs_list_merch_t_name_family_statuses` to log all the changes in the table.
# - Create a trigger `logs_list_merch_t_name_family_statuses_insert` to automatically log INSERT operations on the table `list_merch_t_name_family_statuses`.
# - Create a trigger `logs_list_merch_t_name_family_statuses_update` to automatically log UPDATE operations on the table `list_merch_t_name_family_statuses`.
# - Create a trigger `logs_list_merch_t_name_family_statuses_delete` to automatically log DELETE operations on the table `list_merch_t_name_family_statuses`.
# - Insert some sample data in the table `list_merch_t_name_family_statuses`.
# 
# Constaints:
...
# - Logs of each changes in this table are recorded in the table `logs_list_merch_t_name_family_statuses`
#
...
# Create the table `list_merch_t_name_family_statuses`
CREATE TABLE `list_merch_t_name_family_statuses` (
  ...
  PRIMARY KEY (`uuid`,`merch_t_name_family_status`),
...
# Make sure that a UUID is generated each time a new record is created in the table `list_merch_t_name_family_statuses`.
CREATE TRIGGER `uuid_list_merch_t_name_family_statuses`
  BEFORE INSERT ON `list_merch_t_name_family_statuses`
  FOR EACH ROW
  SET new.uuid = uuid()
;

# Create the table `logs_list_merch_t_name_family_statuses` to store the changes in the data
CREATE TABLE `logs_list_merch_t_name_family_statuses` (
  ...
  `merch_t_name_family_status` varchar(50) COLLATE utf8mb4_unicode_520_ci  NOT NULL COMMENT 'Designation',
  `merch_t_name_family_status_description` text COLLATE utf8mb4_unicode_520_ci COMMENT 'Description/help text',
  KEY `list_merch_t_name_family_statuses_uuid` (`uuid`) COMMENT 'Index the UUID for improved performances'
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_520_ci ROW_FORMAT=DYNAMIC
;

# After a successful INSERT in the table `list_merch_t_name_family_statuses`
# Record all the data Inserted in the table `list_merch_t_name_family_statuses`
# The information will be stored in the table `logs_list_merch_t_name_family_statuses`

DELIMITER $$

CREATE TRIGGER `logs_list_merch_t_name_family_statuses_insert` AFTER INSERT ON `list_merch_t_name_family_statuses`
FOR EACH ROW
BEGIN
  INSERT INTO `logs_list_merch_t_name_family_statuses` (
    `action`, 
    `action_datetime`, 
    `uuid`, 
  ...
  VALUES('INSERT', NOW(), NEW.`uuid`, NEW.`interface_id_creation`, NEW.`interface_id_update`, NEW.`is_obsolete`, NEW.`order`, NEW.`is_active`, NEW.`merch_t_name_family_status`, NEW.`merch_t_name_family_status_description`)
  ;
END
$$
...
# After a successful UPDATE in the table `list_merch_t_name_family_statuses`
# Record all the values for the old record
# Record all the values for the new record
# data Inserted in the table `list_merch_t_name_family_statuses`
# The information will be stored in the table `logs_list_merch_t_name_family_statuses`

DELIMITER $$

CREATE TRIGGER `logs_list_merch_t_name_family_statuses_update` AFTER UPDATE ON `list_merch_t_name_family_statuses`
FOR EACH ROW
BEGIN
  INSERT INTO `logs_list_merch_t_name_family_statuses` (
    `action`, 
    ...
    VALUES
    ('UPDATE-OLD_VALUES', NOW(), OLD.`uuid`, OLD.`interface_id_creation`, OLD.`interface_id_update`, OLD.`is_obsolete`, OLD.`order`, OLD.`is_active`, OLD.`merch_t_name_family_status`, OLD.`merch_t_name_family_status_description`),
    ('UPDATE-NEW_VALUES', NOW(), NEW.`uuid`, NEW.`interface_id_creation`, NEW.`interface_id_update`, NEW.`is_obsolete`, NEW.`order`, NEW.`is_active`, NEW.`merch_t_name_family_status`, NEW.`merch_t_name_family_status_description`)
  ;
END
$$
...
# After a successful DELETE in the table `list_merch_t_name_family_statuses`
# Record all the values for the old record
# The information will be stored in the table `logs_list_merch_t_name_family_statuses`

DELIMITER $$

CREATE TRIGGER `logs_list_merch_t_name_family_statuses_delete` AFTER DELETE ON `list_merch_t_name_family_statuses`
FOR EACH ROW
BEGIN
  INSERT INTO `logs_list_merch_t_name_family_statuses` (
    `action`, 
    ...
    VALUES
    ('DELETE', NOW(), OLD.`uuid`, OLD.`interface_id_creation`, OLD.`interface_id_update`, OLD.`is_obsolete`, OLD.`order`, OLD.`is_active`, OLD.`merch_t_name_family_status`, OLD.`merch_t_name_family_status_description`)
  ;
END
$$
...
INSERT  INTO `list_merch_t_name_family_statuses`(
    `interface_id_creation`, 
    `is_obsolete`, 
    `order`, 
    `is_active`, 
