...
# - Insert some sample data in the table `list_product_statuses`.
# 
# Constaints:
...
CREATE TABLE `list_product_statuses` (
  ...
  PRIMARY KEY (`uuid`,`product_status`),
...
# Make sure that a UUID is generated each time a new record is created in the table `list_product_statuses`.
CREATE TRIGGER `uuid_list_product_statuses`
  BEFORE INSERT ON `list_product_statuses`
  FOR EACH ROW
  SET new.uuid = uuid()
;
...
CREATE TABLE `list_product_statuses_logs` (
  ...
  `product_status` varchar(50) COLLATE utf8mb4_unicode_520_ci  NOT NULL COMMENT 'Designation',
  `product_status_description` text COLLATE utf8mb4_unicode_520_ci COMMENT 'Description/help text',
  KEY `list_product_statuses_uuid` (`uuid`) COMMENT 'Index the UUID for improved performances'
...
BEGIN
  ...
  VALUES('INSERT', NOW(), NEW.`uuid`, NEW.`interface_id_creation`, NEW.`interface_id_update`, NEW.`is_obsolete`, NEW.`order`, NEW.`is_active`, NEW.`product_status`, NEW.`product_status_description`)
  ;
END
$$
...
# After a successful UPDATE in the table `list_product_statuses`
...
BEGIN
  INSERT INTO `list_product_statuses_logs` (
    ...
    VALUES
    ('UPDATE-OLD_VALUES', NOW(), OLD.`uuid`, OLD.`interface_id_creation`, OLD.`interface_id_update`, OLD.`is_obsolete`, OLD.`order`, OLD.`is_active`, OLD.`product_status`, OLD.`product_status_description`),
    ('UPDATE-NEW_VALUES', NOW(), NEW.`uuid`, NEW.`interface_id_creation`, NEW.`interface_id_update`, NEW.`is_obsolete`, NEW.`order`, NEW.`is_active`, NEW.`product_status`, NEW.`product_status_description`)
  ;
END
$$
...
BEGIN
  INSERT INTO `list_product_statuses_logs` (
    ...
    VALUES
    ('DELETE', NOW(), OLD.`uuid`, OLD.`interface_id_creation`, OLD.`interface_id_update`, OLD.`is_obsolete`, OLD.`order`, OLD.`is_active`, OLD.`product_status`, OLD.`product_status_description`)
  ;
END
$$
...
INSERT  INTO `list_product_statuses`(
    `interface_id_creation`, 
    `is_obsolete`, 
    `order`, 
    `is_active`, 
