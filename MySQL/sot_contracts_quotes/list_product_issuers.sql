...
CREATE TABLE `list_product_issuers` (
  ...
  PRIMARY KEY (`uuid`),
  UNIQUE KEY `unique_product_issuer_designation` (`product_issuer`) COMMENT 'The designation must be unique',
  KEY `product_issuer_created_interface_id` (`created_interface_id`),
  KEY `product_issuer_updated_interface_id` (`updated_interface_id`),
  CONSTRAINT `product_issuer_created_interface_id` FOREIGN KEY (`created_interface_id`) REFERENCES `db_interfaces` (`uuid`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `product_issuer_updated_interface_id` FOREIGN KEY (`updated_interface_id`) REFERENCES `db_interfaces` (`uuid`) ON DELETE CASCADE ON UPDATE CASCADE
...
BEGIN
  ...
  VALUES
    ('INSERT', 
      ...
      NEW.`order`, 
      NEW.`product_issuer`, 
      NEW.`product_issuer_description`
    )
  ;
END
$$
  ...
  INSERT INTO `logs_list_product_issuers` (
    ...
    VALUES
      ('UPDATE-OLD_VALUES', 
        ...
        OLD.`order`,  
        OLD.`product_issuer`, 
        OLD.`product_issuer_description`
      ),
      ('UPDATE-NEW_VALUES', 
        NOW(), 
        NEW.`uuid`, 
