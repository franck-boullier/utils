...
CREATE TABLE `list_product_rules` (
  ...
  PRIMARY KEY (`uuid`,`product_rule`),
  KEY `product_rule_interface_id_creation` (`interface_id_creation`),
  KEY `product_rule_interface_id_update` (`interface_id_update`),
  CONSTRAINT `product_rule_interface_id_creation` FOREIGN KEY (`interface_id_creation`) REFERENCES `db_interfaces` (`uuid`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `product_rule_interface_id_update` FOREIGN KEY (`interface_id_update`) REFERENCES `db_interfaces` (`uuid`) ON DELETE CASCADE ON UPDATE CASCADE
...
BEGIN
  ...
  VALUES('INSERT', NOW(), NEW.`uuid`, NEW.`interface_id_creation`, NEW.`interface_id_update`, NEW.`is_obsolete`, NEW.`order`, NEW.`product_rule`, NEW.`product_rule_description`)
  ;
END
$$
...
BEGIN
  INSERT INTO `logs_list_product_rules` (
    ...
    VALUES
    ('UPDATE-OLD_VALUES', NOW(), OLD.`uuid`, OLD.`interface_id_creation`, OLD.`interface_id_update`, OLD.`is_obsolete`, OLD.`order`, OLD.`product_rule`, OLD.`product_rule_description`),
    ('UPDATE-NEW_VALUES', NOW(), NEW.`uuid`, NEW.`interface_id_creation`, NEW.`interface_id_update`, NEW.`is_obsolete`, NEW.`order`, NEW.`product_rule`, NEW.`product_rule_description`)
  ;
END
$$
...
BEGIN
  INSERT INTO `logs_list_product_rules` (
    ...
    VALUES
    ('DELETE', NOW(), OLD.`uuid`, OLD.`interface_id_creation`, OLD.`interface_id_update`, OLD.`is_obsolete`, OLD.`order`, OLD.`product_rule`, OLD.`product_rule_description`)
  ;
END
$$
