...
# - Create a table `db_interfaces_logs` to log all the changes in the table.
...
# - Logs of each changes in this table are recorded in the table `db_interfaces_logs`
#
...
# Create the table `db_interfaces_logs` to store the changes in the data
CREATE TABLE `db_interfaces_logs` (
...
# The information will be stored in the table `db_interfaces_logs`
...
FOR EACH ROW
BEGIN
  INSERT INTO `db_interfaces_logs` (
    `action`, 
    `action_datetime`, 
    `uuid`, 
    `is_obsolete`, 
...
INSERT  INTO `db_interfaces`(`is_obsolete`,`order`,`interface_designation`,`interface_description`) 
  VALUES 
    ...
    (0,20,'sql_script','The update was done with a SQL script.')
;